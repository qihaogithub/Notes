...
var LinkerSettingTab = class extends import_obsidian5.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian5.Setting(containerEl).setName("包含标题").setDesc("If activated, headers (so your lines beginning with at least one `#`) are included for virtual links.").addToggle((toggle) => toggle.setValue(this.plugin.settings.includeHeaders).onChange(async (value) => {
      await this.plugin.updateSettings({ includeHeaders: value });
    }));
    new import_obsidian5.Setting(containerEl).setName("匹配单词的任何部分").setDesc("如果停用，仅匹配整个单词。否则，单词的每个部分都会被找到。").addToggle((toggle) => toggle.setValue(this.plugin.settings.matchAnyPartsOfWords).onChange(async (value) => {
      await this.plugin.updateSettings({ matchAnyPartsOfWords: value });
      this.display();
    }));
    if (!this.plugin.settings.matchAnyPartsOfWords) {
      new import_obsidian5.Setting(containerEl).setName("Match the beginning of words").setDesc("如果激活，即使不是完整匹配，单词的开头也会被链接。").addToggle((toggle) => toggle.setValue(this.plugin.settings.matchBeginningOfWords).onChange(async (value) => {
        await this.plugin.updateSettings({ matchBeginningOfWords: value });
        this.display();
      }));
      new import_obsidian5.Setting(containerEl).setName("匹配单词的结尾").setDesc("如果激活，即使不是完整匹配，单词的结尾也会被链接。").addToggle((toggle) => toggle.setValue(this.plugin.settings.matchEndOfWords).onChange(async (value) => {
    ...
    if (this.plugin.settings.matchAnyPartsOfWords || this.plugin.settings.matchBeginningOfWords) {
      new import_obsidian5.Setting(containerEl).setName("抑制子词的后缀").setDesc("如果激活，后缀不会添加到子词的链接中，只会添加到完整匹配的链接中。").addToggle((toggle) => toggle.setValue(this.plugin.settings.suppressSuffixForSubWords).onChange(async (value) => {
    ...
    new import_obsidian5.Setting(containerEl).setName("大小写敏感").setHeading();
    new import_obsidian5.Setting(containerEl).setName("Case sensitive").setDesc("如果激活，匹配将区分大小写。").addToggle((toggle) => toggle.setValue(this.plugin.settings.matchCaseSensitive).onChange(async (value) => {
    ...
    new import_obsidian5.Setting(containerEl).setName("匹配的文件").setHeading();
    new import_obsidian5.Setting(containerEl).setName("包含所有文件").setDesc("为虚拟链接器包含所有文件。").addToggle((toggle) => toggle.setValue(this.plugin.settings.includeAllFiles).onChange(async (value) => {
    ...
    new import_obsidian5.Setting(containerEl).setName("始终显示多个引用").setDesc("如果开启，当有多个匹配的笔记时，所有引用都会显示在匹配项后面。如果未开启，只有在悬停在匹配项上时才会显示引用。").addToggle((toggle) => toggle.setValue(this.plugin.settings.alwaysShowMultipleReferences).onChange(async (value) => {
      await this.plugin.updateSettings({ alwaysShowMultipleReferences: value });
    }));
    new import_obsidian5.Setting(containerEl).setName("虚拟链接后缀").setDesc("添加到自动生成的虚拟链接的后缀。").addText((text) => text.setValue(this.plugin.settings.virtualLinkSuffix).onChange(async (value) => {
      await this.plugin.updateSettings({ virtualLinkSuffix: value });
    }));
    new import_obsidian5.Setting(containerEl).setName("别名的虚拟链接后缀").setDesc("添加到自动生成的别名虚拟链接的后缀。").addText((text) => text.setValue(this.plugin.settings.virtualLinkAliasSuffix).onChange(async (value) => {
      await this.plugin.updateSettings({ virtualLinkAliasSuffix: value });
    }));
    new import_obsidian5.Setting(containerEl).setName("应用默认链接样式").setDesc("If toggled, the default link styling will be applied to virtual links. Furthermore, you can style the links yourself with a CSS-snippet affecting the class `virtual-link`. (Find the CSS snippet directory at Appearance -> CSS Snippets -> Open snippets folder)").addToggle((toggle) => toggle.setValue(this.plugin.settings.applyDefaultLinkStyling).onChange(async (value) => {
      await this.plugin.updateSettings({ applyDefaultLinkStyling: value });
    }));
    new import_obsidian5.Setting(containerEl).setName("转换时使用默认链接样式").setDesc("如果开启，默认链接样式将用于将虚拟链接转换为实际链接。").addToggle((toggle) => toggle.setValue(this.plugin.settings.useDefaultLinkStyleForConversion).onChange(async (value) => {
