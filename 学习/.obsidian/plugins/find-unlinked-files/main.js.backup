...
var SettingsTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian2.Setting(containerEl).setName("打开输出文件").addToggle(
      (cb) => cb.setValue(this.plugin.settings.openOutputFile).onChange((value) => {
        this.plugin.settings.openOutputFile = value;
        this.plugin.saveSettings();
      })
    );
    new import_obsidian2.Setting(containerEl).setName("输出文件名").setDesc(
      "设置输出文件名（不带扩展名）。请确保不存在具有此名称的文件，因为它将被覆盖！如果名称为空，则设置默认名称。"
    ).addText(
    ...
    new import_obsidian2.Setting(containerEl).setName("禁用工作链接").setDesc(
      "缩进线条以禁用链接并清理图形视图"
    ).addToggle(
      (cb) => cb.onChange((value) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("排除给定目录中的文件").setDesc(
      "启用排除给定目录中的文件。禁用仅包含给定目录中的文件"
    ).addToggle(
